...
def get_top_screener_candidates(all_stock_data: Dict, holdings: List[Dict], settings: Dict) -> List[Dict]:
    ...
    # 보유 종목 제외
    held_tickers = {str(h.get("pdno", "")).zfill(6) for h in holdings}
...
def analyze_rebalance_with_gpt(
    ...
    try:
        ...
        # GPT 프롬프트 구성
        prompt = build_rebalance_prompt(holdings_info, candidates_info, market_trend, analysis_date)
        
        # OpenAI API 호출
        response = _call_openai_json(
            system_prompt="당신은 전문 포트폴리오 매니저입니다. 한국 주식시장의 리밸런싱 결정을 내려야 합니다.",
            user_prompt=prompt
        )
    ...
    for decision in decisions:
        ...
        if action == "SELL":
            ...
            for holding in holdings:
                if str(holding.get("pdno", "")).zfill(6) == ticker:
                    to_sell_list.append({
                        "ticker": ticker,
                        "name": name,
            ...
            for candidate in candidates:
                if str(candidate.get("Ticker", "")).zfill(6) == ticker:
                    to_buy_plans.append({
