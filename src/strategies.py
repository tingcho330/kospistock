...
class BaseStrategy:
    def __init__(self, settings: Settings):
        ...
        # Phase 1: config에서 읽기 (우선순위 적용)
        self.stop_loss_pct = (
            self.params.get("stop_loss_pct") or 
            self.risk_params.get("stop_pct") or 
            0.03  # 최종 기본값
        )
        self.take_profit_pct = (
            self.params.get("take_profit_pct") or 
            self.risk_params.get("auto_sell", {}).get("target_pct") or 
            0.08  # 최종 기본값
        )
...
class RsiReversalStrategy(BaseStrategy):
    def decide_sell(self, holding: Dict, stock_info: Dict) -> bool:
        # 개선된 RSI 전략이 활성화되어 있으면 이 전략은 비활성화 (중복 방지)
        rsi_strategy = self.risk_params.get("rsi_sell_strategy", {})
        if rsi_strategy.get("enabled", False):
            # 개선된 전략이 risk_manager에서 처리하므로 여기서는 False 반환
            return False
        
        # 기존 로직 유지 (하위 호환성)
        rsi_threshold = self.risk_params.get("rsi_take_profit", 75.0)  # 70.0 → 75.0
...
class TrendFollowingStrategy(BaseStrategy):
    def decide_sell(self, holding: Dict, stock_info: Dict) -> bool:
        ...
        # MA200이 없을 때는 MA50만으로 판단 (단기 추세 고려)
        if ma200 <= 0:
            # MA200이 없으면 현재가와 MA50 비교로 단기 추세 판단
            current_price = _to_float_safe(stock_info.get("Price"))
            return current_price < ma50 * 0.95  # 5% 하락 시 판매
        
        # 정상적인 경우: MA50 < MA200 (하락 추세)
        return ma50 < ma200
